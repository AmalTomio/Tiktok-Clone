<template>
  <div
    id="SideNavMain"
    :class="route.fullPath === '/' ? 'lg:w-[310px]' : 'lg:w-[220px]'"
    class="fixed z-20 bg-white pt-[15px] h-full lg:border-r-0 w-[75px] overflow-auto"
  >
    <div class="lg:w-full w-[55px] mx-auto">
      <nuxt-link to="/">
        <MenuItem
          iconString="For You"
          colorString="#F02C56"
          sizeString="30"
        ></MenuItem>
      </nuxt-link>
      <MenuItem
        iconString="Following"
        colorString="#000000"
        sizeString="27"
      ></MenuItem>
      <MenuItem
        iconString="LIVE"
        colorString="#000000"
        sizeString="27"
      ></MenuItem>

      <div class="border-b lg:ml-2 mt-2"></div>
      <div
        class="lg:block hidden text-xs text-gray-600 font-semibold pt-4 pb-2 px-2"
      >
        Suggested Accounts
      </div>
      <div class="lg:hidden block pt-3"></div>

      <div class="cursor-pointer"><MenuItemFollow></MenuItemFollow></div>

      <button class="lg:block hidden text-[#F02C56] pt-1.5 pl-2 text-[13px]">
        See all
      </button>

      <div class="border-b lg:ml-2 mt-2"></div>

      <div
        class="lg:block hidden text-xs text-gray-600 font-semibold pt-4 pb-2 px-2"
      >
        Following Accounts
      </div>

      <div class="lg:hidden block pt-3"></div>

      <div class="cursor-pointer"><MenuItemFollow></MenuItemFollow></div>

      <button class="lg:block hidden text-[#F02C56] pt-1.5 pl-2 text-[13px]">
        See more
      </button>

      <div class="lg:block hidden border-b lg:ml-2 mt-2"></div>

      <div class="lg:block hidden text-[11px] text-gray-500">
        <div class="pt-4 px-2">
          About Newsroom Tiktok Shop Contact Careers by ByteDance
        </div>
        <div class="pt-4 px-2">
          Tiktok for Good Advertise Developers Transparency Tiktok Rewards
          Tiktok Browse
        </div>
        <div class="pt-4 px-2">
          Help Safety Terms and Privacy Creator Protal Community Guidelines
        </div>
        <div class="pt-4 px-2">&copy; 2023 Tiktok</div>
      </div>

      <div class="pb-14"></div>
    </div>
  </div>
</template>

<script setup>
const route = useRoute();
</script>
